...
## W11 — Autonomous personality evolution (added after Interview Round 6)
**Gated on**: Round 6 (DR-020 through DR-023).
**Hypothesis**: personality evolution should be entirely self-authored, with no external approval step of any kind — `LearningReviewQueue` is the wrong abstraction (a queue exists for a reviewer to inspect; there is no reviewer by design) and should be removed or radically simplified, not merely wired up.
**Plan**: `ReflectionService._consolidate_persona` calls `evolve_persona()` directly once a proposal clears the 0.8 confidence bar (DR-023, unchanged) and the `LearningGovernor`'s protected/constitutional-key check (unchanged — this is the immutable-core boundary from DR-001, not "approval," and stays absolute). No rate limit beyond the confidence bar and the existing 5-trait cap (DR-021). No product-facing change log (DR-022) — `evolve_persona`'s internal debug logging may stay for engineering troubleshooting, but nothing user-visible.
**Scope extension from DR-004/DR-011**: baseline mood/PAD drift (previously fixed at persona-authoring time) needs the same self-authored mechanism, on its own much slower timescale (DR-011: weeks-long patterns only).
**Tests**: the governor's protected-key check needs to be airtight now that it's the *sole* remaining gate — this is exactly where W11 should get an adversarial pass (attempt to construct a reflection proposal that touches an immutable/constitutional field through an edge case the governor's key-matching misses). BrainBench's personality suite measures drift for evaluation purposes (reading state directly in the test harness), which doesn't conflict with DR-022's opacity decision since that's an evaluation concern, not a product feature.
