...
def aggregate(
    ...
    """Per-category mean and 95% bootstrap interval for every metric.
        ...
        out[cat] = {
            ...
            **{
                name: {
                    ...
                    "ci95": [round(x, 4) for x in _bootstrap_ci(vals)],
                }
...
def paired_delta(
    ...
    """Paired bootstrap of mean(b - a) over probes present in both runs."""
    index_a = {(r.scenario_seed, r.probe_key): r.metrics()[metric] for r in a}
    pairs = [
        (index_a[(r.scenario_seed, r.probe_key)], r.metrics()[metric])
        for r in b
    ...
    if not diffs:
        return {"n": 0}
    lo, hi = _bootstrap_ci(diffs, seed=seed, n=n)
    return {
        "n": len(diffs),
