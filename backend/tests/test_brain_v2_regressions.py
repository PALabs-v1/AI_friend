...
@pytest.mark.asyncio
async def test_brain_honours_stop_addressed_to_the_superseded_reply():
    agent = _brain(active="turn-new", superseded="turn-old")
    await agent._on_audio_stop(_confirmed_stop("turn-old"))
    agent._cancel_active_generation.assert_awaited_once()
    agent._truncate_interrupted_reply.assert_awaited_once()
